...
function pickNumber(next, prev) {
  return Number.isFinite(Number(next)) ? Number(next) : prev ?? null;
}

function pickString(next, prev) {
  if (typeof next !== "string") return prev ?? null;
  const t = next.trim();
  return t.length ? t : (prev ?? null);
}
...
export async function saveDealData(deal, args) {
  try {
    ...
    // Build a “safe merged” object: if args doesn’t provide it, keep prior value.
    const merged = {
