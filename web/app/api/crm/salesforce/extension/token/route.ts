...
import {
  signSalesforceExtensionToken,
  type SalesforceExtensionTokenPayload,
} from "../../../../../../lib/salesforceExtensionJwt";

function jsonError(status: number, msg: string) {
  return NextResponse.json({ ok: false, error: msg }, { status });
}

function toDealState(row: any) {
  const health_pct =
  ...
  return {
    ...
    baseline_health_score:     row.baseline_health_score     ?? null,
    health_pct,
...
export async function POST(req: Request) {
  try {
    ...
    // Resolve org from Salesforce org ID
    ...
    const oppRes = await pool.query(
      `SELECT
       ...
       LEFT JOIN users u_req   ON u_req.id = o.review_requested_by
      WHERE o.crm_opp_id = $1
        AND o.org_id = $2
      LIMIT 1`,
      [opportunityId, org_id]
    );
    if (!oppRes.rows[0]) {
      return jsonError(404, "Opportunity not found — run a sync from SalesForecast.io admin settings");
    }
    ...
    const [reviewToken, dashboardToken] = await Promise.all([
      signSalesforceExtensionToken({ ...basePayload, purpose: "review" }, 3600),
      signSalesforceExtensionToken({ ...basePayload, purpose: "dashboard" }, 3600),
    ]);
    ...
    console.error("[sf-extension:token]", e);
