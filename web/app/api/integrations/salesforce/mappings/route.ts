...
export async function GET() {
  ...
  const { rows } = await pool.query(
    `
    SELECT sf_field, sfdc_api_name, confidence::text AS confidence, is_active
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    for (const m of mappings) {
      ...
      const apiName = m?.sfdc_api_name == null ? null : String(m.sfdc_api_name).trim() || null;
      const conf = String(m?.confidence || "none").trim();
      const confOk = ["high", "medium", "low", "none"].includes(conf) ? conf : "none";
      if (!sf) continue;
      await client.query(
        `
        INSERT INTO salesforce_field_mappings (org_id, sf_field, sfdc_api_name, confidence, is_active)
  ...
  // Apply mappings to the Salesforce mapping set so the ingest pipeline
  // picks up the latest field configuration immediately.
      ...
      if (queue && QUEUE_NAME === "opportunity-ingest") {
        try {
          await queue.add(
            ...
            { orgId: org.orgId, syncLogId, syncType: "manual", crm: "salesforce" },
            {
              jobId: `salesforce-manual-sync_${org.orgId}_${syncLogId}`,
              attempts: 3,
          ...
          await pool
            .query(
              `UPDATE salesforce_sync_log SET status = 'failed', completed_at = now(), error_text = 'Queue unavailable' WHERE id = $1::uuid`,
              [syncLogId]
            )
            .catch(() => {});
        }
