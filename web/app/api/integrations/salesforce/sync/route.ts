...
async function resolveOrgId(
  ...
  if (auth.kind === "user") {
    ...
    return { ok: true, orgId: auth.user.org_id };
  }
  const mid = getMasterOrgIdFromCookies();
  if (!mid) {
    return { ok: false, status: 400, error: "Select an active organization first." };
  }
  return { ok: true, orgId: mid };
}
...
export async function GET() {
  ...
  const o = await resolveOrgId(auth);
  if (o.ok === false) {
    return NextResponse.json({ ok: false, error: o.error }, { status: o.status });
  }
  ...
  return NextResponse.json({
    status: r.status,
    opportunities_fetched: Number(r.opportunities_fetched || 0),
    opportunities_upserted: Number(r.opportunities_upserted || 0),
    opportunities_scored: Number(r.opportunities_scored || 0),
...
export async function POST() {
  ...
  // In-flight guard — prevent duplicate syncs
  ...
  if (!queue || QUEUE_NAME !== "opportunity-ingest") {
    await pool.query(
      ...
      UPDATE salesforce_sync_log
       ...
       WHERE id = $1::uuid
      `,
      [syncLogId, "Redis queue unavailable (REDIS_URL)."]
    );
    return NextResponse.json(
      { ok: false, error: "Redis queue unavailable" },
      { status: 503 }
    );
  }
  ...
  try {
    await queue.add(
      ...
      { orgId: o.orgId, syncLogId, syncType: "manual", crm: "salesforce" },
      {
        jobId: `salesforce-manual-sync_${o.orgId}_${syncLogId}`,
        attempts: 3,
