...
export default async function AnalyticsComparisonsPage({
  ...
  const company = selected ? await getCompanyAttainmentForPeriod({ orgId, quotaPeriodId }).catch(() => null) : null;
  const reps = selected ? await listRepAttainmentForPeriod({ orgId, quotaPeriodId, limit: 200 }).catch(() => []) : [];
  const deals = selected ? await listStageComparisonsForPeriod({ orgId, quotaPeriodId, limit: 200, onlyMismatches }).catch(() => []) : [];
  ...
  return (
    <main>
      ...
      <section className="mt-4 rounded-xl border border-[color:var(--sf-border)] bg-[color:var(--sf-surface)] p-5 shadow-sm">
        ...
        <form method="GET" action="/admin/analytics/comparisons" className="mt-3 grid gap-3 md:grid-cols-3">
