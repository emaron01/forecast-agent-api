  ...
  return (
    <main className="grid gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-slate-900">Sales Org Chart</h1>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        {saved ? (
          <div className="mb-4 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">Saved.</div>
        ) : null}
        {error ? (
        ...
        <form action={updateSalesOrgChartAction} className="grid gap-5">
          <div className="flex items-center justify-end gap-2">
            <button className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white">Save org chart</button>
          </div>
          ...
          {executives.length ? (
            <div className="grid gap-4">
              {executives.map((exec) => {
                ...
                return (
                  <section key={exec.id} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      <div className="text-xs text-slate-500">{exec.public_id}</div>
                    </div>
                    ...
                    <div className="mt-4 grid gap-3">
                      {myManagers.length ? (
                          ...
                          return (
                            <div key={m.id} className="rounded-lg border border-slate-200 bg-white p-3">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                  <label className="text-xs font-medium text-slate-600">Managed by</label>
                                  <select
                              ...
                              <div className="mt-2 text-xs text-slate-500">{m.email}</div>

                              <div className="mt-3">
                                <div className="text-xs font-semibold text-slate-700">Reps</div>
                                {myReps.length ? (
                                  <ul className="mt-2 grid gap-2">
                                    {myReps.map((r) => (
                                      <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-slate-200 px-3 py-2">
                                        <div>
                                  ...
                                  <div className="mt-2 text-sm text-slate-500">No reps assigned.</div>
                                )}
                              </div>
                            </div>
                          );
                        ...
                        <div className="text-sm text-slate-500">No managers assigned to this executive.</div>
                      )}
                    </div>
                  </section>
                );
            ...
            <div className="text-sm text-slate-600">No Executive Managers found. Create an Executive Manager user first.</div>
          )}

          {(unassignedManagers.length || unassignedReps.length) ? (
            <section className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              <div className="text-sm font-semibold text-amber-900">Unassigned</div>
              <p className="mt-1 text-xs text-amber-800">These users do not currently have a manager assigned.</p>

              {unassignedManagers.length ? (
                <div className="mt-3">
                  <div className="text-xs font-semibold text-amber-900">Managers</div>
                  <ul className="mt-2 grid gap-2">
                    {unassignedManagers.map((m) => (
                      <li key={m.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-amber-200 bg-white px-3 py-2">
                        <div>
                          <div className="text-sm text-slate-900">{m.display_name}</div>
                          <div className="text-xs text-slate-500">{m.email}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <label className="text-xs font-medium text-slate-600">Managed by</label>
                          <select name={`mgr_${m.public_id}`} defaultValue="" className="rounded-md border border-slate-300 px-2 py-1 text-sm">
                            <option value="">(unassigned)</option>
                            {executives.map((e) => (
                              <option key={e.public_id} value={String(e.public_id)}>
                                {e.display_name}
              ...
              {unassignedReps.length ? (
                <div className="mt-3">
                  <div className="text-xs font-semibold text-amber-900">Reps</div>
                  <ul className="mt-2 grid gap-2">
                    {unassignedReps.map((r) => (
                      <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-amber-200 bg-white px-3 py-2">
                        <div>
                          <div className="text-sm text-slate-900">{r.display_name}</div>
                          <div className="text-xs text-slate-500">{r.email}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <label className="text-xs font-medium text-slate-600">Managed by</label>
                          <select name={`mgr_${r.public_id}`} defaultValue="" className="rounded-md border border-slate-300 px-2 py-1 text-sm">
                            <option value="">(unassigned)</option>
                            {managers.map((m) => (
