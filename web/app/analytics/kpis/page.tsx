        ...
        {!visiblePeriods.length ? (
          ...
          <div className="mt-5 grid gap-4">
            {visiblePeriods.map((p) => {
              ...
              return (
                <details key={p.id} open={block.is_current} className="rounded-xl border border-[color:var(--sf-border)] bg-[color:var(--sf-surface)] shadow-sm">
                  <summary className="cursor-pointer px-5 py-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-semibold text-[color:var(--sf-text-primary)]">
                          {summaryLabel} {block.is_current ? <span className="ml-2 rounded bg-[color:var(--sf-surface-alt)] px-2 py-0.5 text-xs">Current</span> : null}
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 text-xs">
                        <div className="rounded-md border border-[color:var(--sf-border)] bg-[color:var(--sf-surface-alt)] px-3 py-2">
                          <div className="text-[color:var(--sf-text-secondary)]">Attainment</div>
                          <div className="font-mono font-semibold text-[color:var(--sf-text-primary)]">{fmtPct(block.attainment)}</div>
                        </div>
                        <div className="rounded-md border border-[color:var(--sf-border)] bg-[color:var(--sf-surface-alt)] px-3 py-2">
                        ...
                        <div className="rounded-md border border-[color:var(--sf-border)] bg-[color:var(--sf-surface-alt)] px-3 py-2">
                          ...
                          <div className="font-mono font-semibold text-[color:var(--sf-text-primary)]">{fmtPct(block.win_rate)}</div>
                        </div>
                        <div className="rounded-md border border-[color:var(--sf-border)] bg-[color:var(--sf-surface-alt)] px-3 py-2">
                          <div className="text-[color:var(--sf-text-secondary)]">New Pipeline</div>
                      ...
                      <table className="w-full min-w-[1100px] text-left text-sm">
                        <thead className="bg-[color:var(--sf-surface-alt)] text-xs text-[color:var(--sf-text-secondary)]">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-right">new pipeline</th>
                        ...
                        <tbody>
                          {block.managers.length ? (
                            block.managers.map((m) => {
                              const managerAov = safeDiv(m.won_amount, m.won_count);
                              const managerPartnerPct = safeDiv(m.partner_closed_amount, m.closed_amount);
                              return (
                                <tr key={`${p.id}:${m.manager_id || "unassigned"}`} className="border-t border-[color:var(--sf-border)] align-top">
                                    ...
                                    <details className="group">
                                      <summary className="cursor-pointer list-none">
                                        <div className="flex items-center justify-between gap-3">
                                          <div className="font-medium text-[color:var(--sf-text-primary)]">{m.manager_name}</div>
                                          <div className="text-xs text-[color:var(--sf-text-secondary)] group-open:hidden">Expand reps</div>
                                          <div className="text-xs text-[color:var(--sf-text-secondary)] hidden group-open:block">Collapse reps</div>
                                        </div>
                                      </summary>
                                      <div className="mt-3 overflow-auto rounded-md border border-[color:var(--sf-border)]">
                                        <table className="w-full min-w-[980px] text-left text-sm">
                                          <thead className="bg-[color:var(--sf-surface-alt)] text-xs text-[color:var(--sf-text-secondary)]">
                                            <tr>
                                              <th className="px-3 py-2">rep (sorted by won)</th>
                                  ...
                                  <td className="px-4 py-3 text-right font-mono text-xs">
                                    {fmtMoney(m.created_amount)}{" "}
                                    <span className="text-[color:var(--sf-text-secondary)]">({m.created_count})</span>
                                  </td>
                                  <td className="px-4 py-3 text-right font-mono text-xs">{fmtMoney(managerAov)}</td>
                                  <td className="px-4 py-3 text-right font-mono text-xs">{fmtPct(managerPartnerPct)}</td>
                                </tr>
                              );
                            })
                          ) : (
