    ...
    <div className="grid gap-4">
      <section className="w-full rounded-2xl border border-[color:var(--sf-border)] bg-[color:var(--sf-surface)] p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-3">
          ...
          {/* TEST (visual only): condensed hero KPI layout (does not replace existing cards) */}
          {(() => {
            ...
            const Row = (p: { label: string; value: React.ReactNode }) => (
              <div className="grid grid-cols-[minmax(140px,1fr)_auto] items-center gap-3">
                ...
                <div className="text-right font-mono text-sm font-semibold text-[color:var(--sf-text-primary)]">{p.value}</div>
              </div>
            );

            return (
              <div className="rounded-xl border border-[color:var(--sf-border)] bg-[color:var(--sf-surface-alt)] p-4 shadow-sm">
                <div className="text-xs font-semibold uppercase tracking-wide text-[color:var(--sf-text-secondary)]">TEST (visual only) — Condensed HERO KPIs</div>
                <div className="mt-3 grid gap-2">
                  <Row
                    label="Closed Won (QTD)"
                    value={
                      <span>
                        {fmtShortMoney(curRev)}{" "}
                        <span className="text-[color:var(--sf-text-secondary)]">
                          {arrow(rev.d)}
                          {fmtShortMoney(rev.d)} vs LQ
                        </span>
                      </span>
                    }
                  />
                  <Row
                    ...
                    value={
                      <span>
                        {curOrders.toLocaleString()}{" "}
                        <span className="text-[color:var(--sf-text-secondary)]">
                          {arrow(ord.d)}
                          {ord.d > 0 ? "+" : ord.d < 0 ? "-" : ""}{Math.abs(Math.trunc(ord.d)).toLocaleString()} vs LQ
                        </span>
                      </span>
                    }
                  />
                  <Row
                    ...
                    value={
                      <span>
                        {fmtShortMoney(curAcv)}{" "}
                        <span className="text-[color:var(--sf-text-secondary)]">
                          {arrow(acv.d)}
                          {fmtShortMoney(acv.d)} vs LQ
                        </span>
                      </span>
                    }
                  />
                  <Row label="Opp→Win" value={fmtPct(oppToWin)} />
                  <Row
                    label="Health (Won/Loss)"
                    value={
                      <span>
                        <span className={healthColorClass(avgHealthWon)}>{avgHealthWon == null ? "—" : `${avgHealthWon}%`}</span>{" "}
                  ...
                  />
                  <Row label="Aging (Won)" value={props.quarterKpis?.wonAvgDays == null ? "—" : `${Math.round(props.quarterKpis.wonAvgDays)}d`} />
                </div>
              </div>
            );
          })()}
