...
export async function handleFunctionCall({ toolName, args, pool }) {
  ...
  // Also allow top-level fields when provided by the tool.
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, org_id, forecast_stage, ai_forecast, risk_summary, health_score
         FROM opportunities
        WHERE org_id = $1 AND id = $2
        LIMIT 1`,
      [orgId, opportunityId]
    ...
    const auditId = await insertAuditEvent(client, {
      ...
      totalScore: recomputed.total_score ?? null,
      maxScore: 30,
