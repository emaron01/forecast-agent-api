...
wss.on("connection", async (twilioWs) => {
  ...
  // Hard guard: OpenAI allows only ONE active response at a time.
  // If one is active, queue a single follow-up and wait for response.done.
  if (responseInProgress) {
    responseCreateQueued = true;
    if (DEBUG_AGENT) console.log(`[DEBUG_AGENT] response.create BLOCKED (in progress) (${reason})`);
    return;
  }
  ...
  // Guard: never send response.create if a response is already active or we haven't
  ...
  responseInProgress = true; // ✅ set before sending to avoid races
