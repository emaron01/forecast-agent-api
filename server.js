...
<Response>
  <Connect>
    <Stream url="wss://${req.headers.host}/?org_id=${orgId}&rep_name=${encodeURIComponent(
            repName
        )}" />
  </Connect>
</Response>`);
    } catch (err) {
        console.error("❌ /agent lookup failed:", err.message);
        res
            .type("text/xml")
            .send(
                `<Response><Connect><Stream url="wss://${req.headers.host}/?org_id=1&rep_name=Team" /></Connect></Response>`
            );
    }
});
