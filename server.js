...
const OPENAI_API_KEY = process.env.MODEL_API_KEY; // Kept your working key
...
wss.on("connection", async (ws) => {
  ...
  const attemptLaunch = async () => {
      ...
      const sessionUpdate = {
        ...
        session: {
          ...
          tools: [{
              type: "function",
              name: "save_deal_data",
              description: "Saves scores, tips, and summaries. ALL FIELDS ARE OPTIONAL.",
              parameters: {
                type: "object",
                properties: {
                ...
                required: [] // [FIX 1: THIS WAS THE TRAP. IT IS NOW EMPTY.]
              },
          }],
          tool_choice: "auto",
        },
      ...
      // I kept your setTimeout since you know it works for you
...
const handleFunctionCall = async (args) => {
    ...
    try {
        ...
        // [FIX 2: UPDATED SQL TO INCLUDE THE 4 NEW FIELDS & RUN COUNT]
        await pool.query(
            `UPDATE opportunities SET 
              ...
              champion_name=$31, champion_title=$32, rep_comments=$33, manager_comments=$34,
            ...
            [
              ...
              args.risk_summary, args.next_steps, newStage,
              args.champion_name, args.champion_title, args.rep_comments, args.manager_comments,
              deal.id
            ]
        );
        ...
        if (currentDealIndex >= dealQueue.length) {
            ...
            openAiWs.send(JSON.stringify({
                type: "response.create",
                response: { 
                    instructions: `Say: 'Okay, saved. We have ${remaining} deals left. Next is ${nextDeal.account_name}. What is the latest update?'` 
                }
            }));
        }
    } catch (err) {
...
server.listen(PORT, () => console.log(`🚀 Matthew Live on port ${PORT}`));