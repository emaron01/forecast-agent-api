-- When CRM owner (rep_name) changes on an existing opportunity, re-resolve reps.id
-- using the same precedence as the integer wrapper backfill:
--   crm_owner_name, then rep_name, then display_name (case-insensitive, trimmed).
-- If no rep matches, keep rep_id unchanged. INSERT behavior unchanged.
